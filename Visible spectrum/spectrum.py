...
def plot_wavelength_frequency(data, filename):
    ...
    image = _draw_labels(image, "Spectre visible", "Longueur d'onde (nm)", "Fréquence (THz)")
    ...
    for item in data:
        ...
        draw.rectangle(xy=[x-column_width, column_top, x, column_bottom ],
                       fill=item["rgb"])
        x += column_width
...
def plot_frequency_wavelength(data, filename):
    ...
    image = _draw_labels(image, "Spectre visible", "Fréquence (THz)", "Longueur d'onde (nm)")
...
def _draw_labels(image, heading_text, x_axis_text, y_axis_text):
    ...
    heading_text_size = draw.textlength(text=heading_text, font=heading_font)
    draw.text(xy=((image.width / 2)-(heading_text_size / 2), 8),
              text=heading_text,
              align="center",
              font=heading_font,
              fill=(255, 255, 255))

    x_axis_text_size = draw.textlength(text=x_axis_text, font=axis_font)
    draw.text(xy=((image.width / 2) - (x_axis_text_size / 2), image.height - 24),
              text=x_axis_text,
              font=axis_font,
              fill=(255, 255, 255))
    ...
    draw = ImageDraw.Draw(image)
    y_axis_text_size = draw.textlength(text=y_axis_text, font=axis_font)
    draw.text(xy=((image.width / 2)-(y_axis_text_size / 2), 8),
              text=y_axis_text,
              font=axis_font,
              fill=(255, 255, 255))
...
def _draw_y_axes(image, border_width, y_axis_start, y_axis_end, y_axis_interval):
    ...
    index_font = ImageFont.truetype('FreeSans.ttf', 12)
    ...
    for v in range(y_axis_start, y_axis_end + 1, y_axis_interval):
        draw.line(xy=[y_axis_indices_x_left, y, y_axis_indices_x_right, y],
                  ...
                  width=1)
        v_str = str(v)
        v_str_size = draw.textlength(text=v_str, font=index_font)
        draw.text(xy=[y_axis_indices_x_left - 2 - (v_str_size), y - (v_str_size / 2)],
                  text=v_str,
                  font=index_font,
                  fill=(255, 255, 255))
        y -= y_distance
...
def _draw_x_axes(image, border_width, x_axis_start, x_axis_end, x_axis_interval):
    ...
    for v in range(x_axis_start, x_axis_end + 1, x_axis_interval):
        ...
        v_str = str(v)
        v_str_size = draw.textlength(text=v_str, font=index_font)
        draw.text(xy=[x - (v_str_size / 2), x_axis_indices_y_bottom + 2],
                  text=v_str,
                  font=index_font,
                  fill=(255, 255, 255))
        x += x_distance
